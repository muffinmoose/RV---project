...
#   plot_03_velocity_time.png — hitrost vs čas na zatič
#   plot_04_fatigue.png       — trend utrujenosti (pin 2→8)
#   plot_05_velocity.png      — povprečna hitrost po pacientih
#   plot_06_path.png          — porazdelitev dolžine poti
#   plot_07_learning.png      — krivulja učenja skozi čas
#   plot_08_within_patient.png— variabilnost znotraj pacienta
#   plot_09_path_vs_time.png  — korelacija pot vs čas
#   plot_10_boxplot.png       — boxplot časov po pacientih
#   plot_11_fast_vs_slow.png  — primerjava najhitrejših in najpočasnejših
...
print(f"Naloženih {len(summary)} veljavnih posnetkov od {summary['patient_id'].nunique()} pacientov")

# ── Plot 1: Povprečen čas na zatič po pacientih ───────────────────────────────
...
ax.axvline(patient_means.mean(), color="#ff4444", linewidth=1.5,
           linestyle="--", label=f"Povprečje: {patient_means.mean():.2f}s")
ax.legend(facecolor="#3d3d3d", labelcolor="white", fontsize=9)
style(ax, "Povprečen čas na zatič po pacientih", "Čas (s)", "Pacient")
...
ax.axvline(np.mean(all_durations), color="#ff4444", linewidth=2,
           label=f"Povprečje: {np.mean(all_durations):.2f}s")
ax.axvline(np.median(all_durations), color="#00ffcc", linewidth=2,
           linestyle="--", label=f"Mediana: {np.median(all_durations):.2f}s")
ax.legend(facecolor="#3d3d3d", labelcolor="white", fontsize=9)
style(ax, "Porazdelitev časov vstavljanja zatičev", "Čas (s)", "Število")
save_fig(fig, "plot_02_distribution.png")

# ── Plot 3: Hitrost vs čas na zatič ──────────────────────────────────────────
fig, ax = plt.subplots(figsize=(10, 6))
fig.patch.set_facecolor("#1e1e1e")
...
ax.legend(facecolor="#3d3d3d", labelcolor="white", fontsize=9)
style(ax, "Povprečna hitrost vs čas na zatič",
      "Povprečen čas na zatič (s)", "Povprečna hitrost (m/s)")
...
style(ax, "Trend utrujenosti — povprečen čas po zatiških (±1 std)",
      "Številka zatiča", "Povprečen čas (s)")
save_fig(fig, "plot_04_fatigue.png")

# ── Plot 5: Povprečna hitrost po pacientih ────────────────────────────────────
fig, ax = plt.subplots(figsize=(10, 8))
fig.patch.set_facecolor("#1e1e1e")
...
ax.axvline(patient_vel.mean(), color="#ff4444", linewidth=1.5,
           linestyle="--", label=f"Povprečje: {patient_vel.mean():.3f} m/s")
ax.legend(facecolor="#3d3d3d", labelcolor="white", fontsize=9)
style(ax, "Povprečna hitrost zapestja po pacientih", "Hitrost (m/s)", "Pacient")
save_fig(fig, "plot_05_velocity.png")

# ── Plot 6: Porazdelitev dolžine poti ─────────────────────────────────────────
fig, ax = plt.subplots(figsize=(10, 6))
fig.patch.set_facecolor("#1e1e1e")
...
ax.axvline(summary["total_path_m"].mean(), color="#ff4444", linewidth=2,
           label=f"Povprečje: {summary['total_path_m'].mean():.2f}m")
ax.legend(facecolor="#3d3d3d", labelcolor="white", fontsize=9)
style(ax, "Porazdelitev skupne dolžine poti roke", "Pot (m)", "Število")
save_fig(fig, "plot_06_path.png")

# ── Plot 7: Krivulja učenja ───────────────────────────────────────────────────
...
style(ax, "Krivulja učenja — povprečen čas na zatič po sejah",
      "Številka seje (kronološko)", "Povprečen čas na zatič (s)")
save_fig(fig, "plot_07_learning.png")

# ── Plot 8: Variabilnost znotraj pacienta ─────────────────────────────────────
fig, axes = plt.subplots(1, 2, figsize=(14, 6))
fig.patch.set_facecolor("#1e1e1e")
fig.suptitle("Variabilnost znotraj pacienta", color="white", fontsize=12)
...
style(axes[0], "Std. dev. časa med sejami", "Std (s)", "Pacient")
...
style(axes[1], "Delta utrujenosti (zadnji - prvi zatič)\nRdeče=počasneje, Zeleno=hitreje",
      "Delta (s)", "Pacient")

save_fig(fig, "plot_08_within_patient.png")

# ── Plot 9: Korelacija pot vs čas ─────────────────────────────────────────────
fig, ax = plt.subplots(figsize=(10, 6))
fig.patch.set_facecolor("#1e1e1e")
...
ax.plot(x_line, p(x_line), color="#ff4444", linewidth=1.5,
        linestyle="--", label="Trend")
ax.legend(facecolor="#3d3d3d", labelcolor="white", fontsize=9)
style(ax, "Dolžina poti vs čas na zatič",
      "Skupna pot (m)", "Povprečen čas na zatič (s)")
...
style(ax, "Porazdelitev časa na zatič po pacientih (boxplot)", "Čas (s)", "Pacient")
save_fig(fig, "plot_10_boxplot.png")

# ── Plot 11: Primerjava najhitrejših in najpočasnejših ────────────────────────
...
fig, axes = plt.subplots(1, 2, figsize=(14, 6))
fig.patch.set_facecolor("#1e1e1e")
fig.suptitle("Primerjava 3 najhitrejših in 3 najpočasnejših pacientov",
             color="white", fontsize=12)

for ax, group, title, color in zip(
        axes,
        [top3_fast, top3_slow],
        ["3 najhitrejši", "3 najpočasnejši"],
        ["#44ff88", "#ff4444"]):
    ...
    ax.set_xlabel("Številka zatiča", color="white", fontsize=9)
    ax.set_ylabel("Povprečen čas (s)", color="white", fontsize=9)
    ax.tick_params(colors="white")
    ax.spines[:].set_color("#555555")
    ax.grid(True, color="#444444", linewidth=0.5, alpha=0.5)
...
# ── Končni povzetek ───────────────────────────────────────────────────────────
print(f"\n{'='*50}")
print(f"POVZETEK KOHORTE ({len(summary)} posnetkov, {summary['patient_id'].nunique()} pacientov)")
print(f"{'='*50}")
print(f"  Povprečen čas na zatič:  {summary['mean_duration_s'].mean():.2f} ± {summary['mean_duration_s'].std():.2f} s")
print(f"  Povprečna hitrost:       {summary['mean_velocity'].mean():.3f} ± {summary['mean_velocity'].std():.3f} m/s")
print(f"  Povprečna dolžina poti:  {summary['total_path_m'].mean():.2f} ± {summary['total_path_m'].std():.2f} m")
print(f"  Povprečna utrujenost Δ:  {summary['fatigue_delta_s'].mean():.3f} s (+ = počasneje na koncu)")
print(f"  Najhitrejši pacient:     {summary.groupby('patient_id')['mean_duration_s'].mean().idxmin()}")
print(f"  Najpočasnejši pacient:   {summary.groupby('patient_id')['mean_duration_s'].mean().idxmax()}")